...
not extraction machinery, is the binding constraint. A v0.2 follow-up with
$6\times$ the prompts (all six single-turn BFCL splits, plus 1--2
distractor tools per prompt so the substrate must carry tool
\emph{selection}) moved that constraint and exposed the next one: drift
improved to 88.4\% despite the deliberately harder task, but honest
dev-split probes now stop the descent at 60.7\% of channels (75.3\% held)
after only five epochs. Function calling is no longer memorization-bound
but \emph{step}-bound; the runs solve different tasks and are not directly
comparable, so the v0.1 numbers stand above. For calibration:
...
The run that stress-tested the receipts. Under v0.1 it floored at 33.9\% of
channels with 90.0\% held-out self-match---and the report flagged
catastrophic unmasked drift (55.8\% self-match): the adapter had been
sharpened by the renormalized guardrail KL (\S\ref{sec:failures}), then
polished with no guardrail at all. v0.2 reruns the identical task with the
binned KL, 56 off-task anchor texts, and the guardrail active through
polish: drift falls to 8.4 points (88.3\% against a 96.7\% base
decode-noise ceiling) while the floor \emph{improves} to 27.5\% at the same
fidelity, probe-detected with rollback. Figure~\ref{fig:v02} makes the
v0.1 blindness quantitative: at the same step, the guardrail KL reads
$3\times10^{-4}$ on train batches (apparently perfect) and
$5\times10^{-3}$--$0.15$ on anchors---drift lives precisely where a
train-only guardrail cannot see it.

\paragraph{Data diversity is the dominant knob.} Those 600 training
prompts instantiate one sentence template, and held-out prompts share
it---so ``90\% at 27.5\%'' partly measures template fit. Regenerating the
same capability from 3{,}000 prompts spanning eight sentence forms and
four instruction phrasings---free, because the teacher writes its own
targets---collapses the floor by $14\times$ and removes most of the
remaining drift (Figure~\ref{fig:diversity}), with a flat ${\sim}98\%$
frontier and a verified 1.58B $\to$ 0.21B export. Even this floor was
step-capped. We initially read our four settings as ``capability breadth
tracks substrate size''; this experiment corrects that. Substrate size
measures the capability \emph{as exercised by the prompts}: floors are
upper bounds jointly set by data coverage and compression compute, not
intrinsic capability sizes---and data is the cheapest lever, because
...
A post-release audit found the shipped library \emph{optimistic} where it
should be strict (probes ran on prompts that were also in the gradient
batches, against an assumed-perfect baseline---though even the unmodified
base model reproduces its own batched-bf16 targets only ${\sim}92\%$
verbatim) and \emph{strict} where it should be tolerant (the renormalized
sparse KL, \S\ref{sec:failures}). v0.2 fixes both: binned KL, dev-split
probes scored against the unmasked model at the same step, rollback to the
last passing sparsity, and the grad$\times$act warm-start the library port
had lost. Same task, seed, hardware, and metric:
...
\textbf{0.17B parameters (9.0$\times$)} at the v0.2 floor
