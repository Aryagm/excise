...
descends to a \textbf{1.2\%-channel floor} at 91\% verbatim reproduction.
...
exports a physically smaller model: deleting the masked channels and
...
\caption{\textbf{One joint run dominates the staged pipeline.}
...
Figure~\ref{fig:frontier} shows the resulting frontier;
...
not extraction machinery, is the binding constraint. For calibration:
...
at the same training step, the guardrail KL on \emph{train} batches reads
$3\times10^{-4}$ (apparently perfect) while on anchor batches it reads
$5\times10^{-3}$ to $0.15$---drift lives precisely where the train-batch
guardrail cannot see it. One honest caveat survives both versions: after
polish specializes the adapter to the floor mask, off-floor frontier points
degrade (the 50\%-budget mask reads \emph{below} the floor mask); the floor
point is the deployable artifact, and the frontier should be read as
descent-time capability, not a menu of equally-polished options.

Capability breadth tracks substrate size across all four settings: narrow
skills compress to single-digit percentages, broad structured-generation
capabilities need a third to half the channels, consistent with PRISM's
